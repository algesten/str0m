...
impl ArrivalGroup {
    ...
    ///
    /// ## Return
    /// Returns [`Some`] if a decision was made for this packet, [`None`] if the packet was
    /// ignored, e.g. for being out of order.
    ///
    /// `Some(true)` means the packet did belong to the group and was added to it.
    /// `Some(false)` means the packet did not belong to the group and a new group should be
    /// created.
