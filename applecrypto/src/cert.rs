...
impl Certificate {
    ...
    /// Get a reference to the SecCertificate
    pub fn sec_certificate(&self) -> &SecCertificate {
        &self.certificate
    }
...
mod tests {
    ...
    #[test]
    fn compare_ecdsa_with_openssl() {
        use std::fs;
        
        // Generate cert using our implementation
        let cert = super::Certificate::new_self_signed(true, "cn=TestECDSA").unwrap();
        let der = cert.certificate.to_der();
        
        // Write to file for comparison
        fs::write("/tmp/rust-ec-cert.der", &der).unwrap();
        
        eprintln!("\n📋 Certificate generated by cert.rs:");
        eprintln!("   DER length: {} bytes", der.len());
        eprintln!("   First 32 bytes: {:02x?}", &der[..32.min(der.len())]);
        
        // Parse and display key info
        if let Ok(info) = parse_cert_info(&der) {
            eprintln!("   Signature Algorithm: {}", info.sig_alg);
            eprintln!("   Public Key Algorithm: {}", info.pubkey_alg);
            eprintln!("   Has Basic Constraints: {}", info.has_basic_constraints);
        }
        
        eprintln!("\n💡 To compare with OpenSSL certificate:");
        eprintln!("   openssl x509 -inform DER -in /tmp/rust-ec-cert.der -text -noout");
        eprintln!("   openssl x509 -inform DER -in /tmp/rust-ec-cert.der -outform PEM -out /tmp/rust-ec-cert.pem");
        eprintln!("   openssl x509 -in /tmp/test-ec-cert.pem -text -noout");
    }
    ...
    fn parse_cert_info(der: &[u8]) -> Result<CertInfo, AppleCryptoError> {
        // Very basic parsing to extract algorithm OIDs
        let sig_alg = if der.windows(9).any(|w| w == &[0x06, 0x07, 0x2a, 0x86, 0x48, 0xce, 0x3d, 0x04, 0x03]) {
            "ecdsa-with-SHA256".to_string()
        } else if der.windows(9).any(|w| w == &[0x06, 0x09, 0x2a, 0x86, 0x48, 0x86, 0xf7, 0x0d, 0x01, 0x01, 0x0b]) {
            "sha256WithRSAEncryption".to_string()
        } else {
            "unknown".to_string()
        };

        let pubkey_alg = if der.windows(7).any(|w| w == &[0x06, 0x07, 0x2a, 0x86, 0x48, 0xce, 0x3d, 0x02, 0x01]) {
            "id-ecPublicKey".to_string()
        } else if der.windows(9).any(|w| w == &[0x06, 0x09, 0x2a, 0x86, 0x48, 0x86, 0xf7, 0x0d, 0x01, 0x01, 0x01]) {
            "rsaEncryption".to_string()
        } else {
            "unknown".to_string()
        };

        let has_basic_constraints = der.windows(3).any(|w| w == &[0x06, 0x03, 0x55, 0x1d, 0x13]);
