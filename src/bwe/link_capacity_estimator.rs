...
pub struct LinkCapacityEstimator {
    ...
    /// Duration after which capacity estimate expires and resets
    capacity_estimate_reset_window: Duration,
}
...
impl LinkCapacityEstimator {
    ...
    pub fn new() -> Self {
        Self {
            ...
            capacity_estimate_reset_window: Self::DEFAULT_RESET_WINDOW,
        }
    }
    ...
    pub fn update_from_probe(&mut self, probe_estimate: Bitrate, now: Instant) {
        ...
        self.capacity_estimate = Some(
            self.capacity_estimate
                .map(|current| current.max(probe_estimate))
                .unwrap_or(probe_estimate),
        );
