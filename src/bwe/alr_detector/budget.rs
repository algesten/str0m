...
impl IntervalBudget {
    ...
    pub fn increase_budget(&mut self, delta_time: Duration) {
        let bytes = (self.target_rate * delta_time).as_bytes_i64();
    ...
    pub fn use_budget(&mut self, bytes: DataSize) {
        self.bytes_remaining =
            (self.bytes_remaining - bytes.as_bytes_i64()).max(-self.max_bytes_in_budget);
    }
    ...
    fn calculate_max_bytes(target_rate: Bitrate) -> i64 {
        (target_rate * Self::WINDOW).as_bytes_i64()
    }
...
mod tests {
    ...
    fn test_budget_ratio_clamped() {
        ...
        budget.use_budget(DataSize::bytes((max_bytes * 2) as u64));
        assert_eq!(budget.budget_ratio(), -1.0);
    }
    ...
    fn test_alr_hysteresis_thresholds() {
        ...
        let use_amount =
            (0.30 * (Bitrate::kbps(300) * Duration::from_millis(500)).as_bytes_i64() as f64) as u64;
