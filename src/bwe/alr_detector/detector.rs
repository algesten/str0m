...
/// alr.set_estimated_bitrate(Bitrate::mbps(10));
///
/// // Call for each media packet (not padding/probes)
/// alr.on_bytes_sent(DataSize::bytes(1200), now);
///
/// // Check ALR state
/// if let Some(alr_start) = alr.alr_start_time() {
///     // We've been in ALR since `alr_start`
/// }
/// ```
#[derive(Debug)]
pub struct AlrDetector {
    ...
    /// When we entered ALR (None if not in ALR)
...
impl AlrDetector {
    ...
    pub fn on_bytes_sent(&mut self, bytes: DataSize, now: Instant) {
        ...
        let was_in_alr = self.alr_started_at.is_some();

        if ratio > self.start_budget_level_ratio && !was_in_alr {
        ...
        } else if ratio < self.stop_budget_level_ratio && was_in_alr {
