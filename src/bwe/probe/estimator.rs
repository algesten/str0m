    ...
    pub fn handle_timeout(&mut self, now: Instant) {
        self.states.retain(|s| {
            ...
            if let ProbeResult::Estimate(_) = result {
                // This is already logged at update() time.
            } else {
...
impl ProbeEstimatorState {
    ...
    fn do_calculate_bitrate(&self) -> ProbeResult {
        ...
        let min_bytes = DataSize::bytes(
            (self.config.target_bytes().as_bytes_usize() as f64 * MIN_RECEIVED_BYTES_RATIO) as u64,
        );
...
mod test {
    ...
    fn probe_estimator_lifecycle() {
        ...
        // Start probe
        let config = ProbeClusterConfig::new(1.into(), Bitrate::mbps(2), false);
    ...
    fn lost_probe_packets_do_not_affect_estimate() {
        ...
        let config = ProbeClusterConfig::new(cluster, Bitrate::mbps(2), false);
