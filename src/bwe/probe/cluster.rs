...
pub struct ProbeClusterConfig {
    ...
    /// Whether this probe was created during ALR (Application Limited Region)
    /// ALR probes are used for link capacity estimation.
    is_alr_probe: bool,
}
...
impl ProbeClusterConfig {
    ...
    pub fn new(cluster: TwccClusterId, target_bitrate: Bitrate, is_alr_probe: bool) -> Self {
        Self {
            cluster,
            target_bitrate,
            // WebRTC defaults
...
impl ProbeClusterState {
    ...
    /// Calculate when the next probe packet should be sent.
    ///
    ...
    pub fn next_probe_time(&self) -> Instant {
        ...
        if self.bytes_sent == DataSize::ZERO {
            // No packets sent yet - can send immediately.
            return started_at;
        }
    ...
    ///
    /// Returns `DataSize::ZERO` if it's not time to send yet, or if we've already created
    ...
    pub fn next_packet(&mut self, now: Instant) -> Option<DataSize> {
        ...
        let mut burst_bytes = self.config.target_bitrate * min_delta;

        // Ensure we request at least one padding packet worth of bytes even if min_delta is 0.
        burst_bytes = DataSize::bytes(cmp::max(
            burst_bytes.as_bytes_usize() as u64,
            MAX_PADDING_PACKET_SIZE.as_bytes_usize() as u64,
        ));

        // If we've already met the byte target but not the packet count, continue to request at
        // least one packet per burst.
        let bytes_needed = self.config.target_bytes().saturating_sub(self.bytes_sent);
        let request_bytes = if bytes_needed == DataSize::ZERO {
            MAX_PADDING_PACKET_SIZE
        } else {
            cmp::min(bytes_needed, burst_bytes)
        };
...
mod test {
    ...
    fn probe_cluster_not_complete_before_start() {
        let now = Instant::now();
        let config = ProbeClusterConfig::new(1.into(), Bitrate::mbps(3), false);
