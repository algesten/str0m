...
impl CryptoProvider {
    ...
    pub fn from_feature_flags() -> CryptoProvider {
        ...
        return crate::crypto::rust_crypto::default_provider();

        #[cfg(feature = "openssl")]
        return crate::crypto::openssl::default_provider();

        #[cfg(all(feature = "wincrypto", target_os = "windows"))]
        return crate::crypto::wincrypto::default_provider();
...
/// Test-only default provider, cached and initialized via `from_feature_flags()`.
///
/// This allows tests to use whichever crypto backend is enabled by feature flags.
...
pub fn test_default_provider() -> &'static CryptoProvider {
    ...
    TEST_PROVIDER.get_or_init(CryptoProvider::from_feature_flags)
}
