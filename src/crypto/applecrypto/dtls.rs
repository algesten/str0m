...
use super::cert::{create_sha256_fingerprint, AppleCryptoDtlsCert};

pub struct AppleCryptoDtls(str0m_applecrypto::Dtls);

impl AppleCryptoDtls {
    pub fn new(cert: AppleCryptoDtlsCert) -> Result<Self, super::CryptoError> {
        Ok(AppleCryptoDtls(str0m_applecrypto::Dtls::new(
            cert.certificate.clone(),
        )?))
    }
}

impl DtlsInner for AppleCryptoDtls {
...
fn transform_dtls_event(
    event: str0m_applecrypto::DtlsEvent,
    output_events: &mut VecDeque<DtlsEvent>,
) {
    match event {
        str0m_applecrypto::DtlsEvent::None => {}
        str0m_applecrypto::DtlsEvent::WouldBlock => {}
        str0m_applecrypto::DtlsEvent::Connected {
            srtp_profile_id,
        ...
        str0m_applecrypto::DtlsEvent::Data(vec) => output_events.push_back(DtlsEvent::Data(vec)),
    }
}
