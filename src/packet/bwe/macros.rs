...
macro_rules! log_delay_variation {
    ($($arg:expr),+) => {
        crate::packet::bwe::macros::log_stat!("DELAY_VARIATION", $($arg),+);
    }
}

macro_rules! log_trendline_estimate {
    ($($arg:expr),+) => {
        crate::packet::bwe::macros::log_stat!("TRENDLINE_ESTIMATE", $($arg),+);
    }
}

macro_rules! log_trendline_modified_trend {
    ($($arg:expr),+) => {
        crate::packet::bwe::macros::log_stat!("TRENDLINE_MODIFIED_TREND", $($arg),+);
    }
}

macro_rules! log_bitrate_estimate {
    ($($arg:expr),+) => {
        crate::packet::bwe::macros::log_stat!("BITRATE_ESTIMATE", $($arg),+);
    }
}

macro_rules! log_rate_control_state {
    ($($arg:expr),+) => {
        crate::packet::bwe::macros::log_stat!("RATE_CONTROL_STATE", $($arg),+);
    }
}

macro_rules! log_rate_control_observed_bitrate {
    ($($arg:expr),+) => {
        crate::packet::bwe::macros::log_stat!("RATE_CONTROL_OBSERVED_BITRATE", $($arg),+);
    }
}

macro_rules! log_rate_control_applied_change {
    ($($arg:expr),+) => {
        crate::packet::bwe::macros::log_stat!("RATE_CONTROL_APPLIED_CHANGE", $($arg),+);
    }
}

macro_rules! log_pacer_media_debt {
    ($($arg:expr),+) => {
        crate::packet::bwe::macros::log_stat!("PACER_DEBT", $($arg),+, "media");
    }
}

macro_rules! log_pacer_padding_debt {
    ($($arg:expr),+) => {
        crate::packet::bwe::macros::log_stat!("PACER_DEBT", $($arg),+, "padding");
    }
}
