...
/// bitrate.
pub trait Pacer {
    /// Set the target bitrate. This will achieved either by regular sends or padding.
    /// We can temporarily exceed it if we must drain the queues.
    fn set_target_rate(&mut self, rate: Bitrate);
...
impl Pacer for PacerImpl {
    fn set_target_rate(&mut self, rate: Bitrate) {
        match self {
            PacerImpl::Null(v) => v.set_target_rate(rate),
            PacerImpl::LeakyBucket(v) => v.set_target_rate(rate),
        }
    }
...
impl Pacer for NullPacer {
    fn set_target_rate(&mut self, _rate: Bitrate) {
        // We don't care
    }
...
pub struct LeakyBucketPacer {
    /// The bitrate we're trying to achieve.
    target_rate: Bitrate,
    /// Adjusted target_rate for when we need to drain queues.
    adjusted_rate: Bitrate,
...
impl Pacer for LeakyBucketPacer {
            ...
            match &mut self.padding_to_add {
                p if *p > DataSize::ZERO && send_padding => {
                    ...
                    *p = p.saturating_sub(packet_size);
                    ...
                    trace!("Requested {packet_size} padding");
    ...
    fn register_send(&mut self, now: Instant, packet_size: DataSize, _from: Mid) {
        ...
        self.media_debt = self.media_debt.min(self.adjusted_rate * MAX_DEBT_IN_TIME);
...
impl LeakyBucketPacer {
    pub fn new(target_rate: Bitrate) -> Self {
        const DEFAULT_QUEUE_LIMIT: Duration = Duration::from_secs(2);

        Self {
            target_rate,
            adjusted_rate: Bitrate::ZERO,
    ...
    fn clear_debt(&mut self, elapsed: Duration) {
        self.media_debt = self.media_debt.saturating_sub(self.adjusted_rate * elapsed);
        self.padding_debt = self.padding_debt.saturating_sub(self.target_rate * elapsed);
        crate::packet::bwe::macros::log_pacer_media_debt!(self.media_debt.as_bytes_usize());
    ...
    fn next_action(&self, now: Instant) -> (Instant, Option<&QueueState>, bool) {
        ...
        match (
            ...
            (Some(queue), bitrate, _, _)
                if bitrate > Bitrate::ZERO && self.adjusted_rate > Bitrate::ZERO =>
            {
                // If we have a non-empty queue send on it as soon as possible, possibly waiting
                // for the next pacing interval.
                let drain_debt_time = self.media_debt / self.adjusted_rate;
                    ...
                    && !too_many_padding_packets =>
            {
                ...
                let mut drain_debt_time =
                    (self.media_debt / self.adjusted_rate).max(self.padding_debt / padding_bitrate);
    ...
    fn maybe_update_adjusted_bitrate(&mut self, now: Instant) {
        ...
        if queued_packets == 0 {
            ...
            if should_send_padding {
                ...
                let padding_to_add = self.target_rate * PADDING_BURST_INTERVAL;
    ...
    fn add_padding_debt(&mut self, size: DataSize) {
        ...
        self.padding_debt = self.padding_debt.min(self.target_rate * MAX_DEBT_IN_TIME);
        crate::packet::bwe::macros::log_pacer_padding_debt!(self.padding_debt.as_bytes_usize());
    }
}
...
mod test {
    ...
    fn test_padding_fill_in() {
        ...
        pacer.set_target_rate((15 * 200).into());
        pacer.handle_timeout(now + duration_ms(1), queue.queue_state(now));
        queue.update_average_queue_time(now + duration_ms(1));
