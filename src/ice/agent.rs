...
mod test {
    ...
    /// Regression test: when a pair with nomination in `Attempt` state is replaced by a
    /// redundant candidate, the replacement pair must preserve the nomination so the next
    ...
    fn pair_replacement_preserves_nomination_in_attempt_state() {
        ...
        // Controlled side (camera): receives offer, not controlling
        agent.set_controlling(false);

        // Add a local host candidate (camera's address)
        agent
            .add_local_candidate(Candidate::host(ipv4_1(), "udp").unwrap())
            .unwrap();

        // Mobile initially discovered via peer-reflexive candidate (same address as ipv4_3)
        ...
        agent.add_remote_candidate(prflx);
        assert_eq!(agent.candidate_pairs.len(), 1);

        // Simulate: the pair succeeds its STUN check and gets nominated by the controlling side
        // (mobile sent USE-CANDIDATE). On controlled side this sets Nominated.
        agent.candidate_pairs[0].nominate(false);
        assert!(agent.candidate_pairs[0].is_nominated());

        // The pair sends a binding request, transitioning Nominated → Attempt.
        // The pair is still nominated (is_nominated checks for any non-None state).
        let now = Instant::now();
        let timing = StunTiming::default();
        let _trans_id = agent.candidate_pairs[0].new_attempt(now, &timing);
        assert!(agent.candidate_pairs[0].is_nominated());

        // Now a late trickle ICE candidate arrives — the "real" host candidate for the
        // same remote address, replacing the peer-reflexive pair.
        agent.add_remote_candidate(Candidate::host(ipv4_3(), "udp").unwrap());

        // The pair was replaced. Verify nomination is preserved as Nominated
        // (not Attempt, not None).
        use super::super::pair::NominationState;
        assert_eq!(agent.candidate_pairs.len(), 1);
        assert_eq!(
            agent.candidate_pairs[0].nomination_state(),
            NominationState::Nominated,
            "Replacement pair must reset nomination to Nominated so the next binding \
             cycle can re-establish it.",
        );

        // Drive the replacement pair through a full nomination cycle:
        // Nominated → new_attempt() → Attempt → record_binding_response() → Success
        let pair = &mut agent.candidate_pairs[0];
        let now = now + std::time::Duration::from_millis(100);
        let trans_id = pair.new_attempt(now, &timing);
        assert_eq!(pair.nomination_state(), NominationState::Attempt);

        // Simulate receiving the binding response for this transaction
        let now = now + std::time::Duration::from_millis(50);
        pair.record_binding_response(now, trans_id, 0);
        assert_eq!(
            pair.nomination_state(),
            NominationState::Success,
            "After binding response, nomination must recover to Success.",
        );
    }
}
