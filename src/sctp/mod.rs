...
use sctp_proto::generate_snap_init;
...
///
/// Holds transport parameters and optional SNAP (out-of-band) INIT chunks.
/// The local INIT chunk is lazily generated and cached on first access.
/// The remote INIT chunk can be set for out-of-band signaling (SNAP).
///
/// # Example
/// ```
/// use str0m::channel::SctpConfig;
///
/// let mut config = SctpConfig::new();
///
/// // Get local INIT chunk to send to remote peer
/// let local_init = config.local_init_chunk().expect("valid chunk");
///
...
impl SctpConfig {
    ...
    pub fn local_init_chunk(&mut self) -> Result<Vec<u8>, SctpError> {
        if self.local_sctp_init.is_none() {
            let init = generate_snap_init(&self.transport)?;
            self.local_sctp_init = Some(init.to_vec());
        }
        Ok(self
            .local_sctp_init
            .as_ref()
            .expect("local init set above")
            .clone())
    }

    /// Check if remote INIT chunk has been configured for out-of-band signaling.
    pub fn has_remote_chunk_init(&self) -> bool {
        self.remote_sctp_init.is_some()
    }
    ...
    pub fn set_remote_chunk_init(&mut self, value: Vec<u8>) {
        self.remote_sctp_init = Some(value);
    }

    /// Build a `ClientConfig` from this `SctpConfig`, consuming it.
    ///
    /// Only used internally by [`RtcSctp::init()`] to feed sctp-proto.
    pub(crate) fn into_client_config(self) -> ClientConfig {
        let mut config = ClientConfig::new();
        config.transport = self.transport;
        if let (Some(local), Some(remote)) = (self.local_sctp_init, self.remote_sctp_init) {
...
impl RtcSctp {
    ...
    ///
    /// Returns an error if SCTP has already been initialized.
    pub fn set_config(&mut self, config: SctpConfig) -> Result<(), SctpError> {
        if self.state != RtcSctpState::Uninited {
            return Err(SctpError::Proto(ProtoError::Other(
                "Cannot set SCTP config after init".into(),
            )));
        }
    ...
    /// Get a mutable reference to the SCTP configuration, if one exists.
    ///
    /// Unlike [`Self::sctp_config_or_default()`], this does **not** create a
    /// default config — use it when you need to inspect without side effects.
    ///
    /// Returns `None` if SCTP has already been initialized (the config is
    /// consumed during `init()`) or no config was previously set.
    #[allow(dead_code)]
    pub fn sctp_config(&mut self) -> Option<&mut SctpConfig> {
        if self.state != RtcSctpState::Uninited {
            return None;
        }
        self.sctp_config.as_mut()
    }

    /// Get or create a mutable reference to the SCTP configuration.
    ///
    /// A default config is created if none was previously set. Use this in
    /// code paths that need to generate or accept SNAP parameters.
    ///
    /// Returns `None` if SCTP has already been initialized (the config is
    /// consumed during `init()`).
    pub fn sctp_config_or_default(&mut self) -> Option<&mut SctpConfig> {
        if self.state != RtcSctpState::Uninited {
            return None;
        }
        Some(self.sctp_config.get_or_insert_with(SctpConfig::default))
    }

    pub fn init(&mut self, client: bool, now: Instant) -> Result<(), SctpError> {
        if self.state != RtcSctpState::Uninited {
            return Err(SctpError::Proto(ProtoError::Other(
        ...
        // If remote INIT was provided out-of-band but local INIT was never
        // retrieved, the caller forgot to send their side of the exchange.
        // Error instead of silently generating a chunk that would never
        // reach the peer.
        //
        // Note: in the SDP API path, `resolve_local_sctp_init` generates the
        // local INIT automatically so this guard only fires via the direct API.
        if has_remote_chunk_init && sctp_config.local_sctp_init.is_none() {
            return Err(SctpError::Proto(ProtoError::Other(
                "Remote SCTP INIT set but local INIT was never generated \
                 (call local_init_chunk() before init)"
                    .into(),
            )));
        }
...
mod tests {
    ...
    fn connect_client_server() -> (RtcSctp, RtcSctp) {
        let now = Instant::now();
        let mut client = RtcSctp::new(None);
        let mut server = RtcSctp::new(None);

        client.init(true, now).unwrap();
        server.init(false, now).unwrap();
