...
use str0m::media::{Direction, Dtmf, DtmfEvent, Frequency, MediaKind, MediaTime, Mid, Pt, Rid};
...
pub fn dtmf_sequence() -> Result<(), RtcError> {
    ...
    // Queue "1", "2", "3" at the same caller timestamp. The sender must place
    // them consecutively on the RTP timeline rather than reusing the timestamp.
    ...
    for d in digits {
        l.writer(mid)
            .unwrap()
            .write_dtmf(pt, wallclock, rtp_time, d, Duration::from_millis(80))?;
    }
...
#[test]
pub fn frame_send_rtp_receive_with_rid() -> Result<(), RtcError> {
    init_log();
    init_crypto_default();
    ...
    run_for(&mut l, &mut r, Duration::from_millis(300))?;
    let packets = te_rtp_packets(&r, pt);
    assert!(!packets.is_empty(), "RID-bound DTMF packets were not sent");
    assert!(packets.iter().all(|(_, timestamp, _)| *timestamp == 8000));

    Ok(())
}

#[test]
pub fn frame_send_rtp_receive_flash_has_zero_volume() -> Result<(), RtcError> {
    init_log();
    init_crypto_default();
    ...
    assert!(
        packets
            .iter()
            .all(|(_, _, payload)| payload.event == 16 && payload.volume == 0)
    );
...
#[test]
pub fn rtp_send_frame_receive_times_out_when_all_end_packets_are_lost() -> Result<(), RtcError> {
    init_log();
    init_crypto_default();

    let (mut l, mut r, _mid, ssrc) = connect_dtmf(Mode::Rtp, Mode::Sample);
    let pt = te_pt(&l.rtc);
    let series = dtmf_series(9, 10, Frequency::EIGHT_KHZ, Duration::from_millis(100), 0);

    send_rtp_series(&mut l, &mut r, ssrc, pt, 8000, 1000, &series)?;
    run_for(&mut l, &mut r, Duration::from_millis(300))?;

    let events = dtmf_events(&r);
    assert_eq!(events.len(), 1, "unfinished event was not timed out");
    assert_eq!(events[0].dtmf, Some(Dtmf::D9));
    assert_eq!(events[0].duration.numer(), 640);
...
#[test]
pub fn rtp_send_frame_receive_after_ssrc_change() -> Result<(), RtcError> {
    init_log();
    init_crypto_default();

    let (mut l, mut r, mid, old_ssrc) = connect_dtmf(Mode::Rtp, Mode::Sample);
    let pt = te_pt(&l.rtc);
    let first = dtmf_series(1, 10, Frequency::EIGHT_KHZ, Duration::from_millis(80), 3);
    send_rtp_series(&mut l, &mut r, old_ssrc, pt, 3_000_000, 1000, &first)?;

    let new_ssrc: Ssrc = 2.into();
    l.direct_api()
        .reset_stream_tx(mid, None, new_ssrc, None)
        .expect("declared stream");

    let second = dtmf_series(2, 10, Frequency::EIGHT_KHZ, Duration::from_millis(80), 3);
    for (index, (marker, payload)) in second.iter().enumerate() {
        let wallclock = l.start + l.duration();
        l.direct_api()
            .stream_tx_by_mid(mid, None)
            .expect("reset stream_tx")
            .write_rtp(
                RtpWrite::new(
                    pt,
                    (2000 + index as u64).into(),
                    1_000_000,
                    wallclock,
                    payload.to_bytes(),
                )
