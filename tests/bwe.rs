...
use str0m::{Candidate, Event, Input, Rtc, RtcError};
...
fn connect_with_bwe(
    ...
    // Set desired bitrate immediately after creating RTCs
    ...
    // Initialize Rtc's internal last_now before starting DTLS
    // This prevents "time went backwards" warning from dimpl
    let l_time = l.last;
    let r_time = r.last;
    l.handle_input(Input::Timeout(l_time)).unwrap();
    r.handle_input(Input::Timeout(r_time)).unwrap();
...
pub fn bwe_cellular() -> Result<(), RtcError> {
    ...
    // It seems to converge around 16Mbps after ~30s. The max rate of change is 8%/second
    let expected = Bitrate::mbps(16);
    let tolerance = 0.20;
...
pub fn bwe_wifi() -> Result<(), RtcError> {
    ...
    // WiFi link is 100 Mbps. We're sending 2Mbps and asking for 50 Mbps
    // It seems to converge around 40Mbps after ~40s. The max rate of change is 8%/second
    let expected = Bitrate::mbps(40);
    let tolerance = 0.20;
    let min_expected = Bitrate::bps((expected.as_u64() as f64 * (1.0 - tolerance)) as u64);
    let max_expected = Bitrate::bps((expected.as_u64() as f64 * (1.0 + tolerance)) as u64);
...
pub fn bwe_changing_bandwidth() -> Result<(), RtcError> {
    ...
    let wifi_expected = Bitrate::mbps(17);
    let tolerance = 0.20;
    ...
    let congested_expected = Bitrate::mbps(5);
    let congested_min =
