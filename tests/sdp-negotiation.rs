...
/// §5.6: Re-offer after SNAP establishment must re-send the same `a=sctp-init`
...
fn snap_sdp_renegotiation_preserves_sctp_init() -> Result<(), RtcError> {
    ...
    assert_eq!(
        ...
        "Re-offer must contain the same a=sctp-init as the initial offer (§5.6)"
    );
    ...
    assert_eq!(
        ...
        "Re-answer must contain the same a=sctp-init as the initial answer (§5.6)"
    );
...
fn snap_sdp_malformed_base64_ignored() -> Result<(), RtcError> {
    ...
    let offer_sdp = offer.to_sdp_string();
    // Replace the valid base64 with garbage — parser will drop it as Unused.
    let tampered = replace_sctp_init(&offer_sdp, "!!!not-valid-base64!!!");
    let tampered_offer = SdpOffer::from_sdp_string(&tampered).unwrap();

    // The answerer should NOT reciprocate since the sctp-init was dropped.
