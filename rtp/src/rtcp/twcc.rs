...
impl TwccSendRegister {
    ...
    pub fn send_records(&self, range: RangeInclusive<SeqNo>) -> Option<Vec<&TwccSendRecord>> {
        ...
        // TODO: Cannot implement Step for SeqNo because this is a nightly only API.
        // In the future this code could be `range.zip(current_index..)`.
        // SEE: https://github.com/rust-lang/rust/issues/42168
        for (seq, index) in (start..=end).zip(first_index..) {
            let seq_no: SeqNo = seq.into();
            assert!(seq_no == self.queue[index].seq);

            result.push(&self.queue[index]);
        }
