...
/// Export EC private key components: returns (X, Y, D) byte arrays.
#[allow(clippy::type_complexity)]
pub(crate) fn export_ec_private_key(
    key_handle: BCRYPT_KEY_HANDLE,
    coord_size: usize,
    ...
    unsafe {
        ...
        // BCRYPT_ECCKEY_BLOB header is 8 bytes: dwMagic(4) + cbKey(4)
        // followed by X(cbKey) + Y(cbKey) + D(cbKey)
        let header_size = 8;
        if blob.len() < header_size + 3 * coord_size {
            return Err("Exported private key blob too small".into());
        }

        let x = blob[header_size..header_size + coord_size].to_vec();
        let y = blob[header_size + coord_size..header_size + 2 * coord_size].to_vec();
        let d = blob[header_size + 2 * coord_size..header_size + 3 * coord_size].to_vec();
...
impl ActiveKeyExchange for X25519KeyExchange {
    ...
    fn complete(self: Box<Self>, peer_pub: &[u8], out: &mut Buf) -> Result<(), String> {
        ...
        // CNG's BCRYPT_KDF_RAW_SECRET returns bytes in reversed order.
        // Reverse to match the standard byte order expected by TLS/dimpl.
