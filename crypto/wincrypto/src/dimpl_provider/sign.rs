...
/// Generate a new ECDSA P-256 key pair and return (key_handle, X, Y, D).
#[allow(clippy::type_complexity)]
pub(crate) fn generate_ecdsa_p256_keypair()
-> Result<(Owned<BCRYPT_KEY_HANDLE>, Vec<u8>, Vec<u8>, Vec<u8>), String> {
    // SAFETY: Microsoft Learn documents `BCryptGenerateKeyPair` and
    // `BCryptFinalizeKeyPair` as initializing the caller-provided handle for
    // the duration of the call; the output handle outlives this block.
    ...
    // Export private key components.
    let (x, y, d) = super::kx_group::export_ec_private_key(*key_handle, 32)?;

    Ok((key_handle, x, y, d))
}

/// Sign data with an ECDSA key handle, returning the raw (r || s) signature.
pub(crate) fn ecdsa_sign_hash(
    key_handle: BCRYPT_KEY_HANDLE,
    hash: &[u8],
) -> Result<Vec<u8>, String> {
    let mut sig = vec![0u8; 64]; // P-256 signature: 32 + 32
    let mut sig_len = 0u32;
    // SAFETY: Microsoft Learn documents `BCryptSignHash` as borrowing the key
    // handle, hash, and output buffer only for the duration of the call; all
    // three outlive this block.
    // Docs: https://learn.microsoft.com/windows/win32/api/bcrypt/nf-bcrypt-bcryptsignhash
    unsafe {
        WinCryptoError::from_ntstatus(BCryptSignHash(
            key_handle,
