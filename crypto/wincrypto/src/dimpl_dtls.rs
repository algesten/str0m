...
// ============================================================================
// Public entry points called from dtls.rs
// ============================================================================

pub(crate) fn generate_certificate() -> Option<DtlsCert> {
    match generate_certificate_impl() {
        Ok(cert) => Some(cert),
        Err(e) => {
            eprintln!("wincrypto dimpl: certificate generation failed: {e}");
            None
        }
    }
}
