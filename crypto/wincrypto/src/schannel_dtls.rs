//! DTLS implementation using Windows SChannel (DTLS 1.2 only).

use std::collections::{HashMap, VecDeque};
use std::sync::{Arc, LazyLock, Mutex};
use std::time::{Duration, Instant};

use str0m_proto::crypto::dtls::{DtlsCert, DtlsImplError, DtlsInstance, DtlsOutput};
...
pub(crate) fn new_dtls(
    ...
    if dtls_version != DtlsVersion::Dtls12 {
        return Err(CryptoError::Other(
            "WinCrypto DTLS provider with dtls12 feature only supports DTLS 1.2. \
             Disable the dtls12 feature for DTLS 1.3/Auto support."
                .to_string(),
        ));
    }
    ...
    Ok(Box::new(SchannelDtlsInstance {
        dtls,
...
impl std::fmt::Debug for SchannelDtlsInstance {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SchannelDtlsInstance").finish()
    }
}

impl SchannelDtlsInstance {
...
impl DtlsInstance for SchannelDtlsInstance {
    ...
    fn poll_output<'a>(&mut self, buf: &'a mut [u8]) -> DtlsOutput<'a> {
        ...
        DtlsOutput::Timeout(base_time + Duration::from_millis(100))
    }

    fn handle_timeout(&mut self, now: Instant) -> Result<(), DtlsImplError> {
        self.last_timeout = Some(now);
        // SChannel handles DTLS retransmissions internally, so we don't need to
        // do anything here. The handshake is driven by handle_packet receiving
        // data from the peer.
        Ok(())
    }
