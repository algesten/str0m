...
impl DtlsProvider for OsslDtlsProvider {
    ...
    fn new_dtls(
        ...
        match dtls_version {
            DtlsVersion::Dtls12 => {
                // Use OpenSSL's native DTLS 1.2 implementation
                Ok(Box::new(OsslDtlsInstance::new(cert)?))
            }
            DtlsVersion::Dtls13 | DtlsVersion::Auto => {
                // Use dimpl with OpenSSL as crypto backend for DTLS 1.3/Auto
                new_dimpl_dtls(cert, now, dtls_version, self.is_test())
            }
...
/// Create a dimpl-based DTLS instance with OpenSSL crypto backend.
fn new_dimpl_dtls(
    cert: &DtlsCert,
    now: Instant,
    dtls_version: DtlsVersion,
