...
//!
//! When the `dtls12` feature is enabled (default), uses OpenSSL's native DTLS for DTLS 1.2 only.
//! When disabled, uses dimpl with OpenSSL crypto for DTLS 1.2/1.3/Auto.
...
#[derive(Debug)]
pub(crate) struct OsslDtlsProvider;
...
impl DtlsProvider for OsslDtlsProvider {
    ...
    fn new_dtls(
        ...
        #[cfg(feature = "dtls12")]
        {
            let _ = now; // OpenSSL manages timeouts internally
            crate::ossl_dtls::new_dtls(cert, dtls_version)
        }
        #[cfg(not(feature = "dtls12"))]
        {
