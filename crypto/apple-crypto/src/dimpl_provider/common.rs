...
use str0m_proto::crypto::{dtls::DtlsInstance, CryptoError, DtlsVersion};
...
pub(crate) trait DimplCommonHmacProvider {
...
macro_rules! impl_hmac_providers {
    ($ty:ty) => {
        impl $crate::dimpl_provider::_reexport::HmacProvider for $ty {
            fn hmac_sha256(&self, key: &[u8], data: &[u8]) -> Result<[u8; 32], String> {
                use $crate::dimpl_provider::common::DimplCommonHmacProvider as _;
                let mut out = [0u8; 32];
                self.hmac(
                    $crate::dimpl_provider::_reexport::HashAlgorithm::SHA256,
                    key,
                    data,
                    &mut out,
                )?;
        ...
        impl $crate::dimpl_provider::_reexport::PrfProvider for $ty {
            fn prf_tls12(
                &self,
                secret: &[u8],
                label: &str,
                seed: &[u8],
                out: &mut $crate::dimpl_provider::_reexport::Buf,
                output_len: usize,
                scratch: &mut $crate::dimpl_provider::_reexport::Buf,
                hash: $crate::dimpl_provider::_reexport::HashAlgorithm,
            ) -> Result<(), String> {
                use $crate::dimpl_provider::common::DimplCommonHmacProvider as _;
        ...
        impl $crate::dimpl_provider::_reexport::HkdfProvider for $ty {
            fn hkdf_extract(
                &self,
                hash: $crate::dimpl_provider::_reexport::HashAlgorithm,
                salt: &[u8],
            ...
            fn hkdf_expand_label(
                ...
                let info = $crate::dimpl_provider::common::build_hkdf_label(
                    b"tls13 ", label, context, output_len,
                )?;
            ...
            fn hkdf_expand_label_dtls13(
                ...
                let info = $crate::dimpl_provider::common::build_hkdf_label(
                    b"dtls13", label, context, output_len,
                )?;
                self.hkdf_expand(hash, secret, &info, out, output_len)
            }
        }
        ...
        mod dimpl_common_hmac_tests {
            use super::*;
            use $crate::dimpl_provider::_reexport::{
