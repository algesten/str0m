//! HKDF implementation using Android JNI HMAC for TLS 1.3 key derivation.
...
#[derive(Debug)]
pub(super) struct AndroidHkdfProvider;

/// Compute HMAC for the given hash algorithm.
fn hmac(hash: HashAlgorithm, key: &[u8], data: &[u8]) -> Result<Vec<u8>, String> {
    match hash {
        HashAlgorithm::SHA256 => {
            let result = jni_crypto::hmac_sha256(key, data).map_err(|e| format!("{e:?}"))?;
            Ok(result.to_vec())
        }
        HashAlgorithm::SHA384 => {
            let result = jni_crypto::hmac_sha384(key, data).map_err(|e| format!("{e:?}"))?;
            Ok(result.to_vec())
        }
...
impl HkdfProvider for AndroidHkdfProvider {
    fn hkdf_extract(
        &self,
        hash: HashAlgorithm,
        salt: &[u8],
...
pub(super) static HKDF_PROVIDER: AndroidHkdfProvider = AndroidHkdfProvider;
