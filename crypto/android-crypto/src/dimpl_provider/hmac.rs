//! HMAC implementations using Android JNI crypto.
...
#[derive(Debug)]
pub(super) struct AndroidHmacProvider;

impl HmacProvider for AndroidHmacProvider {
    fn hmac_sha256(&self, key: &[u8], data: &[u8]) -> Result<[u8; 32], String> {
        jni_crypto::hmac_sha256(key, data).map_err(|err| format!("{err:?}"))
    }
}

pub(super) static HMAC_PROVIDER: AndroidHmacProvider = AndroidHmacProvider;
...
mod test {
    ...
    fn test_hmac_sha256_test_case_1() {
        ...
        let provider = AndroidHmacProvider;
